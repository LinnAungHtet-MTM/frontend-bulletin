...
export const useUpdateUser = (userId?: number) => {
  ...
  const [selectedUser, setSelectedUser] = useState<UpdateUserForm | null>(null);
  ...
  useEffect(() => {
    ...
    reset({
      ...
      role: selectedUser.role ? "1" : "0",
      phone: selectedUser.phone ?? "",
      address: selectedUser.address ?? "",
      dob: selectedUser.dob ? selectedUser.dob : undefined,
      profile: null,
    });

    setProfilePreview(selectedUser.profile_path ?? null);
  }, [selectedUser, reset]);
